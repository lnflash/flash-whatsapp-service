...
export class ContextualHelpService {
  ...
  private readonly confusionPatterns: ConfusionPattern[] = [
    // Repeated unknown commands
    {
      ...
      helpSuggestion:
        "I notice you're trying different commands. Type `help` to see what I can do, or tell me what you'd like to accomplish!",
      priority: 1,
    },
    // Multiple failed payment attempts
    {
      ...
      helpSuggestion:
        'Having trouble with payments? Make sure to use: `send [amount] to [username]`. For example: `send 10 to john`',
      priority: 2,
    },
    ...
    // Voice command confusion
    {
      ...
      helpSuggestion:
        'For voice commands: `voice on` enables voice, `voice list` shows available voices, `voice [name]` selects a voice.',
      priority: 1,
    },
    // Contact confusion
    {
      ...
      helpSuggestion:
        'To manage contacts: `contacts add john +1234567890` saves a contact, `contacts list` shows all contacts.',
      priority: 1,
    },
  ...
  async analyzeForConfusion(whatsappId: string): Promise<string | null> {
    ...
    if (unknownCount >= 3) {
      detectedPatterns.push(
        this.confusionPatterns.find((p) => p.pattern === 'multiple_unknown_commands')!,
      );
    }
    ...
    const unlinkedAttempts = activities.filter(
      (a) =>
        a.errorType === 'unlinked' &&
        [CommandType.BALANCE, CommandType.SEND, CommandType.RECEIVE, CommandType.HISTORY].includes(
          a.type,
        ),
    ).length;
    ...
    if (verifyAttempts >= 2) {
      detectedPatterns.push(
        this.confusionPatterns.find((p) => p.pattern === 'verification_issues')!,
      );
    }
    ...
    const voiceConfusion = activities.filter(
      (a) =>
        a.command.toLowerCase().includes('voice') && (a.type === CommandType.UNKNOWN || a.failed),
    ).length;
    if (voiceConfusion >= 2) {
    ...
    const contactConfusion = activities.filter(
      (a) =>
        a.command.toLowerCase().includes('contact') && (a.type === CommandType.UNKNOWN || a.failed),
    ).length;
  ...
  async getSmartSuggestions(
    ...
    switch (currentCommand) {
      ...
      case CommandType.CONTACTS:
        suggestions.push(
          'contacts add name +1234567890',
          'send 10 to contact_name',
          'contacts list',
        );
        break;
